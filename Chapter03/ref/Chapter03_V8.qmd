...
format:
  revealjs:
    ...
    # chalkboard: 
    #   buttons: true          # 在屏幕左下角显示 HUD 图标
    #   boardmarker-width: 3    # 画笔粗细
    #   theme: chalkboard       # 默认黑板风格
    #   background: [ 'rgba(127,127,127,.1)' , path + 'whiteboard.png' ] # 自定义背景
  beamer:
    theme: Madrid
...
2. $x$ 是分段单调的，即存在周期的有限划分，使得 $x$ 在每个区间段上都是单调的（有限个极值点）；
3. 在任何有限区间内，只有有限个不连续点，在这些不连续点上，函数是有限值。
...
- **$\sum_{k \neq 0} \frac{\hat{x}[k]}{j k \omega_{0}}$ 去哪里了?**
...
$\phi_{N}^{k}$ 示例， $N=8$ 和 $k=0,1, \ldots, 8$

![](imglink/finitness.png){fig-align=center}

## 傅里叶系数的确定

周期序列 $x[n]$，其周期为 $N$，$x[n]$可以表示为
$$
x=\sum_{k \in[N]} a_{k} e^{j k \frac{2 \pi}{N} n}
$$这些系数 $a_k$取值如何确定？这个问题实质上就是要求得一组线性联立方程的解。 显然：

$$
\begin{aligned}
x[0] &= \sum_{k \in[N]} a_k \\
x[1] &= \sum_{k \in[N]} a_k e^{j 2\pi k / N} \\
&\vdots \\
x[N-1] &= \sum_{k \in[N]} a_k e^{j 2\pi k (N-1) / N}
\end{aligned} 
$$

这样，当 $k$ 在 $N$ 个连续整数值变化时，对应于 $N$ 个未知系数 $a_k$ 的 $N$ 个线性方程。上面的$N$个方程组相互独立，可以利用$x[0]$，$x[1]$,...$x[N-1]$求出系数$a_k$
...
$\hat{x}[k]$称为$x[n]$的**频谱系数(Spectral Coefficient)**。
...
基频$\omega_0=\frac{2 \pi}{N}=\frac{2 \pi}{5}$
...
其中
$$H(s)=\int_{-\infty}^{+\infty} h(\tau) e^{-s\tau} d \tau  \qquad  H(z)=\sum_{k = - \infty}^{+\infty} h(k) z^{-k}$$

当$s$或$z$是一般复数时，$H(s)$和$H(z)$就称为该系统的**系统函数(System Function)**。

:::{.hud }
本章和下一章：重点在$Rs[s]=0$的特殊情况。即$s=j \omega$。

对于离散时间信号系统，重点在$|z|=1$的$z$值上。即$z=e^{j \omega}$,$z^n=e^{j \omega n}$。
:::

因此，系统函数就简化为
$$H(j \omega)=\int_{-\infty}^{+\infty} h(t)  e^{-j \omega t} d t \qquad H(j \omega)=\sum_{k = - \infty}^{+\infty} H(n) e^{j \omega n}$$

## 线性时不变系统对复指数信号的响应

连续时间系统示例：有一个周期信号 $x(t)$，其基波频率为 $\omega_0 = 2\pi$。该信号可以写成如下傅里叶级数的形式：
$$
x(t) = \sum_{k=-3}^{+3} a_k e^{jk2\pi t}
$$
其中：$a_0 = 1$，$a_1 = a_{-1} = \frac{1}{4}$
，$a_2 = a_{-2} = \frac{1}{2}$，$a_3 = a_{-3} = \frac{1}{3}$
将$x(t)$作为线性时不变（LTI）系统的输入信号，该系统的单位冲激响应为：
...
为了计算输出 $y(t)$ 的傅里叶级数系数，首先要求出系统的响应：

$$
H(j\omega) = \int_{0}^{\infty} e^{-\tau}e^{-j\omega \tau} d\tau = -\left. \frac{1}{1 + j\omega}e^{-\tau}e^{-j\omega \tau} \right|_{0}^{\infty} = \frac{1}{1 + j\omega} 
$$

## 线性时不变系统对复指数信号的响应
### 输出信号 $y(t)$ 的表示

输入信号 $\omega_0 = 2\pi$，因此可得输出信号的表示式为：

$$
y(t) = \sum_{k=-3}^{+3} b_k e^{jk2\pi t} 
$$

由于 $b_k = a_k H(jk2\pi)$，根据输入信号 $a_k$ 的值，可得各谐波系数：

| 谐波次数 $k$ | 正频率分量 $b_k$ | 负频率分量 $b_{-k}$ |
| :--- | :--- | :--- |
| **0** | $b_0 = 1$ | - |
| **1** | $b_1 = \frac{1}{4}\left( \frac{1}{1 + j2\pi} \right)$ | $b_{-1} = \frac{1}{4}\left( \frac{1}{1 - j2\pi} \right)$ |
| **2** | $b_2 = \frac{1}{2}\left( \frac{1}{1 + j4\pi} \right)$ | $b_{-2} = \frac{1}{2}\left( \frac{1}{1 - j4\pi} \right)$ |
| **3** | $b_3 = \frac{1}{3}\left( \frac{1}{1 + j6\pi} \right)$ | $b_{-3} = \frac{1}{3}\left( \frac{1}{1 - j6\pi} \right)$ |

> **注意：** $y(t)$ 一定是实值信号，即 $b_k^* = b_{-k}$，这满足实信号在频域的共轭对称性。
...
**形式一：幅相式**: $y(t) = 1 + 2 \sum_{k=1}^{3} D_k \cos(2\pi kt + \theta_k)$

**形式二：正余弦式**: $y(t) = 1 + 2 \sum_{k=1}^{3} [E_k \cos 2\pi kt - F_k \sin 2\pi kt]$

其中系数关系为：$b_k = D_k e^{j\theta_k} = E_k + jF_k$。

### 数值计算示例 ($k=1$)

对于基波分量，各项参数计算如下：

* **幅度 $D_1$**: $D_1 = |b_1| = \frac{1}{4\sqrt{1 + 4\pi^2}}$
* **相位 $\theta_1$**: $\theta_1 = \angle b_1 = -\arctan(2\pi)$
* **实部 $E_1$**: $E_1 = \text{Re}\{b_1\} = \frac{1}{4(1 + 4\pi^2)}$
* **虚部 $F_1$**: $F_1 = \text{Im}\{b_1\} = -\frac{\pi}{2(1 + 4\pi^2)}$
...
离散时间系统示例：考虑一个线性时不变系统，其单位脉冲响应为 $h[n] = \alpha^n u[n]$，其中 $-1 < \alpha < 1$。输入信号为：
...
系统的响应为单位脉冲响应的离散时间傅里叶变换：

$$
H(e^{j\omega}) = \sum_{n=0}^{\infty} \alpha^n e^{-j\omega n} = \sum_{n=0}^{\infty} (\alpha e^{-j\omega})^n 
$$

利用等比级数求和公式（几何级数收敛条件为 $|\alpha e^{-j\omega}| < 1$），得：
...
利用线性系统的频率响应特性，输出的傅里叶级数为：

$$
\begin{aligned}
y[n] &= \frac{1}{2} H(e^{j2\pi/N}) e^{j(2\pi/N)n} + \frac{1}{2} H(e^{-j2\pi/N}) e^{-j(2\pi/N)n} \\
&= \frac{1}{2} \left( \frac{1}{1 - \alpha e^{-j2\pi/N}} \right) e^{j(2\pi/N)n} + \frac{1}{2} \left( \frac{1}{1 - \alpha e^{j2\pi/N}} \right) e^{-j(2\pi/N)n}
\end{aligned} 
$$

若记频率响应的极坐标形式为：
$$\frac{1}{1 - \alpha e^{-j2\pi/N}} = r e^{j\theta}$$
...
$$
\frac{1}{1 - \alpha e^{-j2\pi/4}} = \frac{1}{1 - \alpha e^{-j\pi/2}} = \frac{1}{1 + \alpha j} = \frac{1}{\sqrt{1 + \alpha^2}} e^{j(-\arctan \alpha)}
$$
...
$\left|H\left(e^{j \omega}\right)\right|$称为**幅值响应 (Magnitude Response):**


$\arg H\left(e^{j \omega}\right)$称为**相位响应 (Phase Response):**
...
**滤波**：改变频率分量的相对幅度的大小，或者完全消除某些频率分量。

- **频率成形滤波器**：用于改变频谱形状的线性时不变系统

- **频率选择性滤波器**：用于基本无失真通过某些频率，而显著衰减或消除另一些频率的线性时不变系统

* **不能**产生新的频率分量。（线性时不变系统）
* **只能**缩放现有分量的幅度或移动其相位。（线性时不变系统）

## 频率成形滤波器  (Frequency-shaping filters) {#sec381}

这类滤波器会改变频谱的形状。

* **例如：均衡器 (Equalizer)**

![](imglink/4cb8e742-a9ce-42fa-a004-42de4559e768-32.png){width=15%,fig-align=center}

## 频率成形滤波器  

![](imglink/EQ.webm){width=10%,fig-align=center}
...
![](imglink/loopshaping.png)

## 频率选择性滤波器
基本无失真通过某些频率，而显著衰减或消除另一些频率的线性时不变系统.
...
$$
H(j \omega)= \begin{cases}1, & \omega_{c 1} \leq|\omega| \leq \omega_{c 2} \\ 0, & \text { 其他s }\end{cases}
$$
...
::: {.column width="40%"}
![](imglink/cityroad.png){width=100% }

![](imglink/cityroadedge.png){width=120%}
:::
...
4. 3.16
5. 3.20
6. 计算机作业：用Python Jupyter 完成程序理解、修改或设计，具体见乐学平台。
