...
Recall the response of a CT LTI system to the input $x(t)=e^{s t}$ is
...
The system function $H(s)$ is called the Laplace transform of $h$.
In general, the Laplace transform of a CT signal $x(t)$ is
...
The set of $s$ for which the integral defining Laplace transform

$$
X(s)=\int_{-\infty}^{\infty} x(t) e^{-s t} d t=\lim _{\substack{T_{1} \rightarrow \infty \\ T_{2} \rightarrow \infty}} \int_{-T_{1}}^{T_{2}} x(t) e^{-s t} d t
$$

converges is called its region of convergence (ROC)
...
If the ROC includes the imaginary axis, setting $\sigma=0$ yields
...
with ROC given by $\operatorname{Re} s>-\operatorname{Re} a$.
