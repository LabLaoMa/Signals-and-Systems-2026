...
其收敛域 $X$ 总是右半平面
注意： 关于 $x(t)$ at $t<0$ 会丢失 in $x$.
注意： $\mathcal{L}\{x\}=\mathcal{U} \mathcal{L}\{x\}$ 当且仅当 $x(t)$ 是因果的, i.e. $x(t)=0$ for $t<0$

## 例子

计算 unilateral Laplace transforms is 与…几乎相同 for bilateral Laplace transforms.
...
$$
\begin{array}{cl}
\text { bilateral } & X_{2}(s)=e^{s} X_{1}(s)=\frac{e^{s}}{s+a}, \quad \operatorname{Re} s>-\operatorname{Re} a \\
\text { unilateral } & X_{2}(s)=\int_{0}^{\infty} e^{-a(t+1)} e^{-s t} d t=\frac{e^{-a}}{s+a}, \operatorname{Re} s>-\operatorname{Re} a
\end{array}
$$

注意： $X_{2} \neq X_{2}$, since $x_{2}$ is noncausal.
注意： $X_{2} \neq e^{s} X_{1}$, 时移性质不成立 for unilateral transform.
...
$$
\begin{aligned}
\text { bilateral } & X_{3}(t)=\frac{-2 a}{s^{2}-a^{2}}, \quad-\operatorname{Re} a<\operatorname{Re} s<\operatorname{Re} a \\
\text { unilateral } & X_{3}(s)=\frac{1}{s+a}, \quad \operatorname{Re} s>-\operatorname{Re} a
\end{aligned}
$$

注意： $X_{3} \neq X_{1}$, since $x_{3} \neq x_{1}$
注意： $X_{3}=X_{1}$, since $x_{3}$ and $x_{1}$ d当且仅当er only for $t<0$
...
注意： The lower limit of integration is $0_{-}$so $\delta$ and $\delta^{\prime}$ contri但是e to $\mathcal{X}$

## 例子

计算 inverse unilateral Laplace transforms is the same as for bilateral Laplace transforms, 但是 we can only recover $x(t)$ for $t \geq 0$ !

例. Given unilateral Laplace transform
...
| D当且仅当erentiation in $s$ domain | $-t x(t)$ | $\frac{d}{d s} X(s)$ |

## Convolution Property of 单边拉普拉斯变换

如果 $x(t)=y(t)=0$ for $t<0$, then
...
and $\lim _{t \rightarrow+\infty} x(t) e^{-s t}=0$ for $s \in R_{0}$, then
...
注意： $R_{0} \supset$ ROAC of $X(s)$ (also true for bilateral Laplace transform)
...
例. $x(t)=e^{-t}, \mathcal{X}(s)=\frac{1}{s+1}$ with ROAC Re $s>-1$. By the d当且仅当erentiation property,
...
例. $x(t)=e^{-t} u(t), X(s)=\frac{1}{s+1}$ with ROAC $\operatorname{Re} s>-1$. By the d当且仅当erentiation property,
...
Under appropriate conditions, e.g. $x^{(k)}(t)=O\left(e^{\gamma t}\right)$, we can extend the d当且仅当erentiation property to higher derivatives,
...
其中 $p_{i}$ are polynomials, the above condition is satisfied.
Unilateral Laplace transform is useful for solving ODEs with nonzero initial condtions.

## 初值定理

定理. 如果 the unilateral Laplace transform $\mathcal{X}(s)$ of $x(t)$ is a proper rational function, then
...
如果 $\mathcal{X}(s)$ is rational 但是 not proper, then

$$
\mathfrak{X}(s)=\sum_{k=0}^{n} a_{k} s^{k}+\mathfrak{X}_{1}(s)
$$

其中 $X_{1}(s)$ is a proper rational function.
...
3. 对于 input $x(t)=1$, the output is $y(t)=0$
...
定理. Suppose $x(t)$ does not contain $\delta$ or its derivatives for $t \geq 0$. 如果 $X(s)$ converges for real $s>0$ and the $x(t)$ has a finite limit as $t \rightarrow+\infty$, then

$$
\lim _{t \rightarrow+\infty} x(t)=\lim _{s \rightarrow 0_{+}} s X(s)
$$

证明. Since $A \triangleq \lim _{t \rightarrow+\infty} x(t)$ exists, $x(t)$ is bounded on $(0,+\infty)$, i.e. $|x(t)| \leq M$. 对于 any $\epsilon>0, \exists T$ s.t. $|x(t)-A|<\epsilon$ for $t \geq T$.
对于 $s>0$,
...
- Analytic functions
- analytic at $z_{0}$ 当且仅当 d当且仅当erentiable on some open disk $B\left(z_{0}, r\right)$
- $f(z)=u(x, y)+j v(x, y)$ is analytic 当且仅当 $u, v$ are continuously d当且仅当erentiable and satisfy Cauchy-Riemann equation
...
- representations of LTI systems (d当且仅当erential/d当且仅当erence equations, block diagrams, impulse response, frequency response, system function)
