...
Theorem. If both $X(s)=\int_{-\infty}^{\infty} x(t) e^{-s t} d t$ and $Y(s)=\int_{-\infty}^{\infty} y(t) e^{-s t} d t$ converges absolutely at some $s=s_{0}$, then the Laplace transform of $z=x * y$ converges absolutely at $s=s_{0}$, and
...
& =\int_{-\infty}^{\infty}\left[\int_{-\infty}^{\infty} x(v) y(t-v) d t\right] e^{-s v} d v \quad \text { (Fubini's Theorem) }
\end{aligned}
$$
...
NB. This property holds also for Laplace transforms of singularity functions and will be useful computing some Laplace transforms.
...
By linearity, $\mathcal{L}^{-1}\{R\}$ is a linear combination of terms of the above form, where signs are chosen according to the ROC.
...
1. If ROC is $\operatorname{Re} s>-1$

$$
x(t)=e^{-t} u(t)-(1+t) e^{-2 t} u(t)
$$

2. If ROC is $\operatorname{Re} s<-2$

$$
x(t)=-e^{-t} u(-t)+(1+t) e^{-2 t} u(-t)
$$

3. If ROC is $-2<\operatorname{Re} s<-1$
...
For $t>0$, choose a large semicircle $C_{r}$ that encloses all $s_{k}$ with $\operatorname{Re} s \leq \sigma_{1}$.

If $X(s)$ satisfies ${ }^{1}$ (a shifted and rotated version of) Jordan's Lemma (slide 6, Lecture 27), Residue Theorem implies
...
where $\sigma_{1}<\sigma<\sigma_{2}$.
For $t<0$, choose a large semicircle $C_{r}$ that encloses all $s_{k}$ with $\operatorname{Re} s \geq \sigma_{2}$.

If $X(s)$ satisfies (a shifted and rotated version of) Jordan's Lemma, Residue Theorem implies
...
1. If ROC is $\operatorname{Re} s>-1$, then $\sigma_{1}=-1, \sigma_{2}=+\infty$.
...
2. If ROC is $\operatorname{Re} s<-2$, then $\sigma_{1}=-\infty, \sigma_{2}=-2$.
...
3. If ROC is $-2<\operatorname{Re} s<-1$, then $\sigma_{1}=-2, \sigma_{2}=-1$.
...
Recall the response of a CT LTI system to the input $x(t)$ is
...
If $x$ and $h$ have Laplace transforms, the convolution property implies
...
If the ROC has a nonempty interior point, the system function (aka transfer function) $H(s)$ uniquely determines $h$ and hence system properties through the inverse Laplace transform (this can be proved, but we will not do so).

[^0]
## 因果性

Recall $h$ is right-sided iff the ROC of $H(s)$ is a right half-plane,
...
Recall an LTI system is stable iff its impulse response $h \in L_{1}$, i.e.
...
A causal LTI system with rational system function $H(s)$ is stable iff all its poles have negative real parts.

例. A causal system with $H(s)=\frac{1}{s+a}$ is stable iff $\operatorname{Re} a>0$
例. A system with $H(s)=\frac{1}{s+a}$ where $\operatorname{Re} a<0$ and ROC $\operatorname{Re} s<-\operatorname{Re} a$ is also stable, but it is noncausal.
...
1. If $\operatorname{Re} s>-3$, causal and stable,

$$
h(t)=\delta(t)-2 e^{-3 t} u(t)
$$

2. If $\operatorname{Re} s<-3$, anticausal and unstable,
...
- Fourier transform method assumes stability, requiring that ROC of $H(s)$ contain the imaginary axis, so $\operatorname{Re} s>-3$
...
1 . If $\operatorname{Re} s>-3$,

$$
y(t)=-2 e^{-3 t} u(t)+3 e^{-4 t} u(t)
$$

2. If $-4<\operatorname{Re} s<-3$,
...
[^0]:    ${ }^{2}$ Actually ROAC for ordinary functions. For most signals in this course, ROC and ROAC coincide, so we will be sloppy.
