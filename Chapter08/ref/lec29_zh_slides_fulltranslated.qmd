...
回顾：连续时间 LTI 系统对输入的响应为 $x(t)=e^{s t}$ is

$$
y(t)=(x * h)(t)=H(s) e^{s t}
$$

其中 $h$ is the impulse response of the system and

$$
H(s)=\int_{-\infty}^{\infty} h(t) e^{-s t} d t
$$

The system function $H(s)$ 称为 the Laplace transform of $h$.
一般来说, the Laplace transform of a CT signal $x(t)$ is
...
使得 $s$ for which the integral defining Laplace transform

$$
X(s)=\int_{-\infty}^{\infty} x(t) e^{-s t} d t=\lim _{\substack{T_{1} \rightarrow \infty \\ T_{2} \rightarrow \infty}} \int_{-T_{1}}^{T_{2}} x(t) e^{-s t} d t
$$

converges 称为 its region of convergence (ROC)
...
如果 $\operatorname{Re} a>0$, the ROC contains the imaginary axis,

$$
\mathcal{F}\{x\}(j \omega)=\left.X(s)\right|_{z=e^{j \omega}}=\frac{1}{j \omega+a}
$$

如果 $\operatorname{Re} a<0$, the CTFT does not exist.
如果 $\operatorname{Re} a=0$, the CTFT exists only as a distri但是ion,
...
D当且仅当erent signals can have the same $X(s)$ 但是 d当且仅当erent ROCs
...
其中 $N, D$ are polynomials that are coprime, i.e. they have no common factors of degree $\geq 1$.
...
- 如果 $n>m, X$ has a pole of order $n-m$ at $\infty$
- 如果 $n<m, X$ has a zero of order $m-n$ at $\infty$
...
注意： The integral in ( $\star$ ) is the unilateral Laplace transform of $x$.

## Convergence of Unilateral 拉普拉斯变换

定理. 如果 the integral in ( $\star$ ) converges for $s=s_{0}=\sigma_{0}+j \omega_{0}$, then it converges for any $s=\sigma+j \omega$ with $\sigma>\sigma_{0}$.
...
定理. 如果 the integral in ( $\star$ ) converges for $s=s_{0}=\sigma_{0}+j \omega_{0}$, then it converges for any $s$ with $\operatorname{Re} s>\sigma_{0}$.
...
In case (c), $\sigma_{c} \in \mathbb{R}$ 称为 the abscissa of convergence, and the line $\operatorname{Re} s=\sigma_{c}$ 称为 the axis of convergence

The $\mathrm{ROC}^{1}$ 总是右半平面
...
定理. 如果 the integral in ( $\star$ ) converges absolutely for $s=s_{0}=\sigma_{0}+j \omega_{0}$, then it converges absolutely and uniformly for $s=\sigma+j \omega$ with $\sigma \geq \sigma_{0}$.
证明.
...
As in the case of convergence, the region of absolute convergence (ROAC) 总是右半平面
...
其中 $\sigma_{a} \in \overline{\mathbb{R}}$ is the abscissa of absolute convergence, and the line $\operatorname{Re} s=\sigma_{a}$ is the axis of absolute convergence.
...
注意： We will use ROC, although in most cases ROC = ROAC.

## 其收敛域 (Bilateral) 拉普拉斯变换
...
is a strip $\sigma_{1}<\operatorname{Re} s<\sigma_{2}$, which is nonempty 当且仅当 $\sigma_{1}<\sigma_{2}$
...
- 如果 $x$ is of finite duration and in $L_{1}$, then the ROC is $\mathbb{C}$.
- 如果 $x$ is left-sided, its ROC is a left half-plane
- 如果 $x$ is right-sided, its ROC is a right half-plane
- 如果 $x$ is two-sided, its ROC is a strip
...
The ROC is nonempty 当且仅当 $b>0$.
注意： 对于 Laplace transform to exist, $x(t)$ has to decay fast enough either as $t \rightarrow+\infty$ or $t \rightarrow-\infty$. The exponential weighting in $e^{-s t}$ cannot kill both.
...
- 如果 $X(s)$ is rational, the ROC is bounded by poles or extends to infinity.
- 如果 $x$ is also right-sided, then its ROC is the right half-plane bounded by the rightmost pole in $\mathbb{C}$, e.g. region III
- 如果 $x$ is also left-sided, then its ROC is the left half-plane bounded by the leftmost pole in $\mathbb{C}$, e.g. region I
...
注意： The ROC may be larger than $R_{1} \cap R_{2}$.
...
ROC enlarges 由于 pole-zero cancellation at $s=-1$.
In time domain,
...
如果 $x$ is real-valued, then $X(s)=X^{*}\left(s^{*}\right)$, so the zeros and poles of $X(s)$ appear in conjugate pairs.
...
证明. D当且仅当erentiation under integral sign (can be justified) 则有
