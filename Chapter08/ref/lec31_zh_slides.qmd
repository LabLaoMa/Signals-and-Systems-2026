...
ROC of $X$ is always a right half-plane
NB. Information about $x(t)$ at $t<0$ is lost in $x$.
NB. $\mathcal{L}\{x\}=\mathcal{U} \mathcal{L}\{x\}$ iff $x(t)$ is causal, i.e. $x(t)=0$ for $t<0$
...
NB. $X_{2} \neq e^{s} X_{1}$, time-shift property fails for unilateral transform.
...
NB. $X_{3} \neq X_{1}$, since $x_{3} \neq x_{1}$
NB. $X_{3}=X_{1}$, since $x_{3}$ and $x_{1}$ differ only for $t<0$
...
NB. The lower limit of integration is $0_{-}$so $\delta$ and $\delta^{\prime}$ contribute to $\mathcal{X}$
...
NB. $R_{0} \supset$ ROAC of $X(s)$ (also true for bilateral Laplace transform)
...
NB. Had we used the definition $X(s)=\int_{0_{+}}^{\infty} x(t) e^{-s t} d t$, we would have $\frac{d}{d t} x(t) \stackrel{\mathcal{L} \mathcal{L}}{\longleftrightarrow} s \mathcal{X}(s)-x\left(0_{+}\right)$
...
Theorem. Suppose $x(t)$ does not contain $\delta$ or its derivatives for $t \geq 0$. If $X(s)$ converges for real $s>0$ and the $x(t)$ has a finite limit as $t \rightarrow+\infty$, then
...
The Final Values Theorem can be extended to allow finitely many $\delta$ and its derivatives in $x(t)$ on the positive real axis. We can rewrite $x(t)$ as
