...
$$\hat{y}(t)=\sum_{k=-\infty}^{+\infty} x(k \Delta) \hat{h}_{k \Delta}(t) \Delta = \sum_{k=-\infty}^{+\infty} x(k \Delta) \hat{h}(t-{k \Delta}) \Delta $$
...
$$y(t)=\int_{-\infty}^{+\infty} x(\tau) h_\tau(t) \mathrm{d} \tau = \int_{-\infty}^{+\infty} x(\tau) h(t-\tau) \mathrm{d} \tau $$
...
&  =\sum_{k=-\infty}^{\infty}   x_{1}[k]\left(x_{2} * x_{3}\right)[n-k] =\left( x_{1} *\left(x_{2} * x_{3}\right)\right)[n]
\end{aligned}
$$
...
例： $x_{1}(t)=1, x_{2}(t)=u(t), x_{3}(t)=\delta^{\prime}(t)$ 

:::: {.columns style="text-align: center;"}
:::{.column width="50%"}

1. $\left(x_{2} * x_{3}\right)(t)=\delta(t)$, 因而 $x_{1} *\left(x_{2} * x_{3}\right)=1$
2. $x_{1} * x_{2}$ 以及 $\left(x_{1} * x_{2}\right) * x_{3}$ 没有定义!

:::
:::{.column width="50%"}
3. $x_{1} * x_{3}=0$, 因而 $\left(x_{1} * x_{3}\right) * x_{2}=0$
:::
::::
...
例子：求由 $y^{\prime}(t)+2 y(t)=x(t)$ 描述的因果 LTI 系统的冲激响应，初始静止条件。
...
- 初始静止 $y(0)=y^{\prime}(0)=0 \Longrightarrow y(t)=t e^{-t}+e^{-2 t}-e^{-t}$
...
关注 $f[n]=0$ 当 $n<n_{0}$ 的情况 $\Longrightarrow$ 初始静止
因果 LTI 系统，输出为
...
第二种定义。由于 $\delta \triangleq \lim _{\Delta \rightarrow 0} r_{\Delta}$，定义
...
$$
\begin{gathered}
\int_{\mathbb{R}} \delta^{\prime}(t) \phi(t)=\lim _{\Delta \rightarrow 0} \int_{\mathbb{R}} r_{\Delta}^{\prime}(t) \phi(t) d t \\
\int_{\mathbb{R}} r_{\Delta}^{\prime}(t) \phi(t) d t=\int_{\mathbb{R}} \frac{1}{\Delta}\left[\delta\left(t+\frac{\Delta}{2}\right)-\delta\left(t-\frac{\Delta}{2}\right)\right] \phi(t) \\
=\frac{1}{\Delta}\left[\phi\left(-\frac{\Delta}{2}\right)-\phi\left(\frac{\Delta}{2}\right)\right] \\
\int_{\mathbb{R}} \delta^{\prime}(t) \phi(t)=-\phi^{\prime}(0)
\end{gathered}
$$
