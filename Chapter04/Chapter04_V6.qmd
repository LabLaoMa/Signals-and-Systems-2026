...
其中 $\Phi(jt) =\frac{1}{2 \pi} \int_{\mathbb{R}} \phi(\omega) e^{-j\omega t} d\omega$。
...
| **右侧第一项** | $x$ | 时域 ($t$) | **原信号分布**。   |
| **右侧第二项** | $\mathcal{F}\phi$ | 时域 ($t$) | **投影测试函数**。计算公式为：$\frac{1}{2 \pi} \int_{-\infty}^{\infty} \phi(\omega) e^{-j\omega t} d\omega$。 |
...
### 另一种对偶形式
$$\langle X, \mathcal{F^{-1}}\Phi \rangle = \langle \mathcal{F^{-1}}X, \Phi \rangle$$ 
...
| **左侧第二项** | $\mathcal{F^{-1}}\Phi$ | 频域 ($\omega$) | **投影测试函数** 计算公式为：$\int_{-\infty}^{\infty} \Phi(t) e^{j\omega t} d t$。  |
...
**方法 2**：使用广义定义。
$$
\frac{1}{2 \pi} \int_{\mathbb{R}} X(j \omega) \phi(\omega) d \omega=\int_{\mathbb{R}} \delta(t) \Phi(j t) d t=\Phi(0)=\int_{\mathbb{R}} 1 \cdot \phi(\omega) d \omega
$$
因此 $X(j\omega)=1$。
...
**方法 2**：使用广义定义。
$$
\frac{1}{2 \pi} \int_{\mathbb{R}} X(j \omega) \phi(\omega) d \omega=\int_{\mathbb{R}} 1 \cdot \Phi(j t) d t=2 \pi \phi(0)=\int_{\mathbb{R}} 2 \pi \delta(\omega) \phi(\omega) d \omega
$$
也就是：
