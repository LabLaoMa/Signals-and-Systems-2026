...
当 $T \to \infty$ 时，$x_T(t) \to x(t) \triangleq u(t + T_1) - u(t - T_1)$，即矩形脉冲 。
...
$$\langle \mathcal{F}^{-1}\{x\}, \phi \rangle \triangleq \langle x, \mathcal{F}^{-1}\{\phi\} \rangle \quad {或} \quad \langle X, \mathcal{F^{-1}}\Phi \rangle = \langle \mathcal{F^{-1}}X, \Phi \rangle$$
...
具体来说，在广义函数（分布）理论中，傅里叶变换（记作 $\mathcal{F}x$）并不是通过直接积分定义的，而是通过其在测试函数空间上的**对偶关系**定义的。

### 核心定义式
$$\langle X(j \omega), \phi(\omega) \rangle \triangleq \langle x(t), \Phi(t) \rangle \quad   : \quad  \langle \mathcal{F}x, \phi \rangle = \langle x, \mathcal{F}\phi \rangle$$ 

其中各项的具体含义如下表：

| 位置 | 符号 | 定义域 | 具体含义与计算形式 |
| :--- | :--- | :--- | :--- |
| **左侧第一项** | $\mathcal{F}x$ | 频域 ($\omega$) | **待求的频谱分布**。 |
| **左侧第二项** | $\phi$ | 频域 ($\omega$) | **测试函数** $\phi(\omega)$ |
| **右侧第一项** | $x$ | 时域 ($t$) | **原信号分布**   |
| **右侧第二项** | $\mathcal{F}\phi$ | 时域 ($t$) | **投影测试函数**。计算公式为：$\Phi(t)=\int_{-\infty}^{\infty} \phi(\omega) e^{-j\omega t} d\omega$ |

> **注意**：右侧的 $\mathcal{F}{\phi}$ 必须使用**傅里叶正变换**（指数项为 $-j\omega t$）

## 基于分布对偶理论的定义(续)

### 另一种对偶形式
$$\langle X(j \omega), \phi(\omega) \rangle \triangleq \langle x(t), \Phi(t) \rangle \quad   :\quad \langle X, \mathcal{F^{-1}}\Phi \rangle = \langle \mathcal{F^{-1}}X, \Phi \rangle$$ 

其中各项的具体含义如下表：

| 位置 | 符号 | 定义域 | 具体含义与计算形式 |
| :--- | :--- | :--- | :--- |
| **左侧第一项** | $X$ | 频域 ($\omega$) | **待求的频谱分布**。 |
| **左侧第二项** | $\mathcal{F^{-1}}\Phi$ | 频域 ($\omega$) | **投影测试函数** 计算公式为：$\phi(\omega)=\frac{1}{2 \pi}\int_{-\infty}^{\infty} \Phi(t) e^{j\omega t} d t$  |
| **右侧第一项** | $\mathcal{F^{-1}}X$ | 时域 ($t$) | **原信号分布**   |
| **右侧第二项** | $\Phi$ | 时域 ($t$) | **测试函数**|

> **注意**：左侧的 $\mathcal{F^{-1}}\Phi$ 必须使用**傅里叶逆变换**（指数项为 $j\omega t$）
...
$$
也就是：
$$2\pi\delta(\omega) = \int_{-\infty}^{\infty} e^{-j\omega t} dt$$
**直流信号的频谱是零频率处的冲激！**
...
$$
\begin{aligned}
\frac{1}{2 \pi} \int_{\mathbb{R}} X(j \omega) \phi(\omega) d \omega & =\int_{\mathbb{R}} e^{j \omega_{0} t} \Phi(j t) d t=2 \pi \phi\left(\omega_{0}\right) \\
...
$$2\pi\delta(\omega) = \int_{-\infty}^{\infty} e^{-j\omega t} dt$$
可以得到：
$$2\pi\delta(\omega - \omega_0) = \int_{\mathbb{R}} e^{j(\omega_0 - \omega)t} dt$$
...
连续时间傅里叶变换（CT Fourier transform）可以被看作是向连续“基”函数空间的**“正交”展开**：

$$X(j\omega) = \langle x, e^{j\omega t} \rangle = \int_{\mathbb{R}} x(t) \overline{e^{j\omega t}} dt$$
$$2\pi\delta(\omega_1 - \omega_2) = \int_{\mathbb{R}} e^{j(\omega_1 - \omega_2)t} dt = \int_{\mathbb{R}} e^{j\omega_1 t} \overline{e^{j\omega_2 t}} dt$$

* $e^{j\omega_1 t}$ 与 $e^{j\omega_2 t}$ 是正交的，如果 $\omega_1 \neq \omega_2$。
...
$$x(t) = \frac{1}{2\pi} \int_{\mathbb{R}} \langle x, e^{j\omega t} \rangle e^{j\omega t} d\omega$$
...
::: {.column width="50%"}
$$
\hat{x}[k]=\frac{\sin(k\omega T_1)}{k \pi}
$$
:::
::: {.column width="50%"}
$$
X(j \omega)=\sum_{k=-\infty}^{\infty}\frac{2 \sin(k\omega T_1)}{k } \delta(\omega-k\omega_0)
$$
:::
::::
...
$$
\mathcal{F}\left\{\tau_{t_{0}} x\right\}=E_{-t_{0}} \mathcal{F}\{x\} \quad \text { 或 } \quad x\left(t-t_{0}\right) \stackrel{\mathcal{F}}{\longleftrightarrow} e^{-j \omega t_{0}} X(j \omega)
$$
...
$$
\mathcal{F}\left\{E_{\omega_{0}} x\right\}=\tau_{\omega_{0}} \hat{x} \quad \text { 或 } \quad e^{j \omega_{0} t} x(t) \stackrel{\mathcal{F}}{\longleftrightarrow} X\left(j\left(\omega-\omega_{0}\right)\right)
$$
...
$$
\mathcal{F}\{R x\}=R \mathcal{F}\{x\} \quad \text { 或 } \quad x(-t) \stackrel{\mathcal{F}}{\longleftrightarrow} X(-j \omega)
$$

共轭

$$
\mathcal{F}\left\{x^{*}\right\}=R \overline{\mathcal{F}\{x\}} \quad \text { 或 } \quad x^{*}(t) \stackrel{\mathcal{F}}{\longleftrightarrow} X^{*}(-j \omega)
$$
...
$$
\mathcal{F}\left\{S_{a} x\right\}=\frac{1}{|a|} S_{\frac{1}{a}} \mathcal{F}\{x\} \quad \text { 或 } \quad x(a t) \stackrel{\mathcal{F}}{\longleftrightarrow} \frac{1}{|a|} X\left(\frac{j \omega}{a}\right)
$$
...
* **播放速度加快（时域压缩）**：**音调变高（Higher pitch）**。

* **播放速度放慢（时域扩展）**：**音调变低（Lower pitch）**。
:::
::::
...
![](imglink/tfscaling.png){width=80%}
:::
::::

:::{.hud }
**实际应用-超宽带(UWB，Ultra-Wideband)测距和定位**： TOF（TOA）测距，测距误差 $\Delta d \approx c / B$（$c$ 为光速，$B$ 为带宽），脉冲持续时间在纳秒级，厘米级定位。。
:::
...
凭直觉判断，$y(t)$ 应该具有直流分量。其平均值 $\bar{y}$ 计算如下：
...
根据积分性质： $\mathcal{F} \{u_{-2}\}(j\omega) = \frac{1}{j\omega}U(j\omega) + \pi U(0)\delta(\omega)$。
...
$U(0) = \frac{1}{j0} + \pi\delta(0)$ 在数学上**没有定义**！$\frac{\pi}{j\omega}\delta(\omega)$在$\omega=0$处如何理解？
...
* 由于 $\frac{2}{j\omega}$ 在 $\omega = 0$ 处不可积，因此 $X$ 应被解释为**主值**，通常记作 $X(j\omega) = \text{pv} \left( \frac{2}{j\omega} \right)$，即：
  ...
  主值分布是通过对称去除奇点邻域，并取极限，从而将本来不可积的奇异函数扩展为分布的一种方式。
...
* $\int_{|\omega| \ge 1} \frac{1}{\omega} \phi(\omega) d\omega$ 是良定义的，因为测试函数 $\phi$ 是快速递减的。
...
* 根据**中值定理**，存在 $\xi$ 使得：$\left| \frac{\phi(\omega) - \phi(0)}{\omega} \right| = |\phi'(\xi)| \le \|\phi\|_{0,1}$

* 因此，我们可以得到以下估计：

$$
\left| \int_{1 \ge |\omega| \ge \epsilon} \frac{1}{\omega} \phi(\omega) d\omega \right| = \int_{1 \ge |\omega| \ge \epsilon} \left| \frac{\phi(\omega) - \phi(0)}{\omega} \right| d\omega \le  \|\phi\|_{0,1}
$$
...
* 设 $x_a(t) = e^{-at}u(t), a > 0$。
<br>
注意当 $a \to 0$ 时，$x_a \to u$ 在**逐点**及**广义函数（分布）意义**下成立 。
...
根据对偶性原理，若 $f(t) \xleftrightarrow{\mathcal{F}} g(\omega)$，则 $g(t) \xleftrightarrow{\mathcal{F}} 2\pi f(-\omega)$。
...
例：$x(t) = \frac{1}{t}$（更准确地表述为 $\text{pv}\left(\frac{1}{t}\right)$）。

已知符号函数的变换：$\text{sgn}(t) \stackrel{\mathcal{F}}{\longleftrightarrow} \frac{2}{j\omega}$

**利用对偶性**


* 交换左侧和右侧，同时交换 $\omega$ 和 $t$ ；

* 执行以下操作（但不同时执行）：

    * 两边取复共轭；

    * 替换 $\omega \to -\omega$ 或 $t \to -t$；

* 将右侧 乘以 $2\pi$ 。
$$\frac{2}{jt} \stackrel{\mathcal{F}}{\longleftrightarrow} 2\pi \text{sgn}(-\omega)$$ 


**利用线性性质**，最终得到变换对：
$$\frac{1}{t} \stackrel{\mathcal{F}}{\longleftrightarrow} -j\pi \text{sgn}(\omega)$$ 

## 对偶性

例：已知 $x(t) = \frac{1}{1+t^2}$ 。

双边衰减指数信号的变换：$e^{-|t|} \stackrel{\mathcal{F}}{\longleftrightarrow} \frac{2}{1+\omega^{2}}$
...
$$
\|x\|_{2}^{2}=\frac{1}{2 \pi}\|X\|_{2}^{2}, \quad \text { 或 } \quad \int_{\mathbb{R}}|x(t)|^{2} d t=\frac{1}{2 \pi} \int_{\mathbb{R}}|X(j \omega)|^{2} d \omega
$$
...
定理。 若 $x, y \in L_{2}(\mathbb{R}), X=\mathcal{F}\{x\}, Y=\mathcal{F}\{y\}$, 
<br>
那么 
$$\langle x, y\rangle=\frac{1}{2 \pi}\langle X, Y\rangle, \quad {或}  \quad \int_{\mathbb{R}} x(t) y^{*}(t) d t=\frac{1}{2 \pi} \int_{\mathbb{R}} X(j \omega) Y^{*}(j \omega) d \omega$$
...
$$
\mathcal{F}\{x * y\}=\mathcal{F}\{x\} \mathcal{F}\{y\}, \quad \text { 或 } \quad(x * y)(t) \stackrel{\mathcal{F}}{\longleftrightarrow} X(j \omega) Y(j \omega)
$$
...
$$
\mathcal{F}\{x y\}=\frac{1}{2 \pi} \mathcal{F}\{x\} * \mathcal{F}\{y\}, \text { 或 } x(t) y(t) \stackrel{\mathcal{F}}{\longleftrightarrow} \frac{1}{2 \pi} \int_{\mathbb{R}} X(j \theta) Y(j(\omega-\theta)) d \theta
$$
...
例 (长除法): 通过长除法，可以将一个假分式有理函数分解为一个多项式与一个真分式之和：
